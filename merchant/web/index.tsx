...
function App({ className }: Props): React.ReactElement<Props> | null {
  ...
  const _onClickFlipContractValue = useCallback(async (): Promise<void> => {
    ...
    await prov.api.isReady
    console.log(111)
    const signer_ = new Signer(prov, accountId, accountSigner);
    // console.log(222)
    // const evmAddress = await signer_.getAddress();
    // console.log(333)
    // const isEvmClaimed = await signer_.isClaimed();


    const balance = await prov.api.derive.balances.all(accountId)
      .then((res) => res.freeBalance.toHuman())
      .then((res) => res === '0' ? '- REEF' : res);
    console.log(balance, BigInt(1e18 * 10))

    const transfer = prov.api.tx.balances.transfer('5EhXnFDwGMDe5MBhAR6z6dFzipbputBz3DK8MLUQCBrrNMJE',BigInt(1e18 * 10))
    let tx = await transfer.signAndSend('5GnLV4GXTsk6hdMq2oyXCFV3FvZnoK3RNEx1B84FTxyVYhQ1');
    console.log(tx)
    // try {
  ...
  useEffect(() => {
    ...
    let genAcc = keyring.encodeAddress(address, SS58_FORMAT);
    console.log('111', genAcc, address);

    if (accountId && evmProvider && evmProvider.api) {
      evmProvider.api.isReady.then(() => {
        evmProvider.api.query.evmAccounts.evmAddresses(genAcc).then((result) => {
          if (result.isEmpty) {
            // setEvmAddress('');
            console.log('222', result)
          } else {
            //setEvmAddress(result.toString());
            console.log("EvmAddress", result.toString());
          }
        });
      });
    }
